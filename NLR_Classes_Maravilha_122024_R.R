...
# This script is to filter out NLRs from specific species
NLR_LOCUS <- read_csv("Locus and Type Legume NLRs 06032024.csv")
Patristic_Distances <- NLR_LOCUS %>%
  filter(Species=="Glycine max" | Species=="Medicago truncatula" | Species=="Lathyrus sativus" |
           Species=="Pisum sativum" |
           Species=="Lens culinaris" | Species=="Cicer arietinum" | Species=="Phaseolus vulgaris") %>%
  arrange(Species, Class)
Medicago_truncatula_NLRs <- NLR_LOCUS %>%
  filter(Species=="Medicago truncatula") %>%
  arrange(Class)
write_csv(Patristic_Distances,"Patristic_Distances_NLRs.csv")
write_csv(Medicago_truncatula_NLRs,"Medicago_truncatula_NLRs.csv")
